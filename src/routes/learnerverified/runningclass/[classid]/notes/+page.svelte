<script lang="ts">
	...
	let { session, supabase, classNow, teacherNow, studentNow, notes } = data;
	$: ({ session, supabase, classNow, teacherNow, studentNow, notes } = data);
	...
	function goEditNote(val) {
		window.open(`/learnerverified/runningclass/${classNow.id}/notespec/${val}/modify`, '_self');
	}

	function goViewNote(val) {
		window.open(`/learnerverified/runningclass/${classNow.id}/notespec/${val}/view`, '_self');
	}
		...
		{#each notes as note}
			<div class="card rounded-lg shadow-lg hover:shadow-2xl p-4">
				...
				<div class="flex flex-row justify-between mt-3">
					...
					{:else if note.owner === studentNow.id}
						<button
							class="btn p-2 bg-cyan-300 hover:bg-cyan-500 rounded-lg"
							on:click={() => goEditNote(note.id)}
							>Edit Note
