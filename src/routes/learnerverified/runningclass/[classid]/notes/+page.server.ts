...
export const load = async ({ params, locals: { supabase, getSession } }) => {
    ...
    let { data: teacher, error: err2 } = await supabase
        .from('teacher')
        .select("*")
    ...
    return { classNow, teacherNow, studentNow, notes };
...
export const actions = {
    upload: async ({ request, locals: { supabase, getSession } }) => {
        ...
        const { data: dt, error: err1 } = await supabase
            .from('classnote')
            .insert([
                { owner: studentNow.id, cid: classNow.id, lang: newClass.lang, title: newClass.title }
            ])


        if (err1) console.log(err1)

        else throw redirect(303, `/learnerverified/runningclass/${classNow.id}/notes`);
    },
